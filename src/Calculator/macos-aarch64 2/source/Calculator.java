...
int displayColor = color(50, 50, 60);
int textColor = color(230, 230, 230);


int blueButtonColor = color(0, 102, 204); 
int redButtonColor = color(204, 51, 51);  
  ...
  switch (command) {
    ...
    case "%":
      if (!displayText.equals("Error")) {
          ...
          displayText = formatResult(num1 / 100.0f);
...
class Button {
  float x, y, w, h;
  String label;
  int baseColor, hoverColor, pressColor;

  Button(float x, float y, float w, float h, String label, int c) {
    this.x = x;
    this.y = y;
    this.w = w;
    this.h = h;
  ...
  public void settings() { size(360, 650); }
