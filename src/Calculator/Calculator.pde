...
color buttonColor = color(70, 70, 80);
color operatorButtonColor = color(255, 140, 0); 
color utilityButtonColor = color(100, 100, 110);
  ...
  for (int row = 0; row < 6; row++) {
    for (int col = 0; col < 4; col++) {
      ...
      if (!label.trim().isEmpty()) {
        ...
        color c = buttonColor; 
        if ("+-/x=".contains(label)) {
          c = operatorButtonColor;
        } else if ("C√x²<--/%+/-".contains(label)) {
          c = utilityButtonColor; 
        }
...
void keyPressed() {
  ...
  if (key == CODED) {
    ...
    switch (key) {
      // 
      case '0': processInput("0"); break;
      case '1': processInput("1"); break;
      case '2': processInput("2"); break;
      case '3': processInput("3"); break;
      case '4': processInput("4"); break;
      case '5': processInput("5"); break;
      case '6': processInput("6"); break;
      case '7': processInput("7"); break;
      case '8': processInput("8"); break;
      case '9': processInput("9"); break;
      case '+': processInput("+"); break;
      case '-': processInput("-"); break;
      case '/': processInput("/"); break;
      case '*': processInput("x"); break;
      case '.': processInput("."); break;
      case '=': processInput("="); break; 
...
void drawDisplay() {
  ...
  while (textWidth(displayText) > width - 50 && currentTextSize > 12) {
      currentTextSize--;
      textFont(createFont("Monospaced.bold", currentTextSize));
  }
...
class Button {
  ...
  Button(float x, float y, float w, float h, String label, color c) {
    ...
    this.label = label;
    this.baseColor = c;
    this.hoverColor = color(min(255, red(c) + 20), min(255, green(c) + 20), min(255, blue(c) + 20)); // Brighter
    this.pressColor = color(max(0, red(c) - 20), max(0, green(c) - 20), max(0, blue(c) - 20)); // Darker
  }
